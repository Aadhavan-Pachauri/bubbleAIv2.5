...
const handleSupabaseError = (error: any, context: string): never => {
    ...
    if (message.includes('schema cache') || message.includes('column') || message.includes('relation')) {
        throw new Error(`Database schema mismatch. The application tried to access a field that doesn't exist in the database. Please refresh the page.`);
    }
...
export const deleteUser = async (supabase: SupabaseClient, userId: string): Promise<void> => {
    // 1. Delete all projects (this handles project-associated chats and messages via manual cascade in deleteProject)
    ...
    // 2. Delete remaining chats (autonomous chats)
    const { data: chats, error: chatsError } = await supabase
        .from('chats')
        .select('id')
        .eq('user_id', userId);
    ...
    if (chats && chats.length > 0) {
        ...
        if (messagesError) handleSupabaseError(messagesError, 'Error deleting user messages');
        ...
        if (deleteChatsError) handleSupabaseError(deleteChatsError, 'Error deleting user chats');
    }
    ...
    if (memoryError) handleSupabaseError(memoryError, 'Error deleting user memories');
...
export const addMessage = async (supabase: SupabaseClient, message: Omit<Message, 'id' | 'created_at'>): Promise<Message> => {
    ...
    // Cast to 'any' to allow deleting arbitrary properties that might cause schema errors
    ...
    // Remove internal agent communication fields that definitely aren't in the DB schema
    delete messageToInsert.memoryToCreate;

    // Map camelCase to snake_case for DB compatibility if needed
    if (messageToInsert.groundingMetadata) {
        // Assuming standard snake_case for DB column. If this column doesn't exist,
        // we should consider deleting it entirely, but mapping is a safe first step if
