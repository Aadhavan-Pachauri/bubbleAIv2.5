...
const DocumentEditor: React.FC<{
    ...
    const handleSave = () => {
        if (!documentFile || !onActiveProjectUpdate) return;
        const newContent = content;
        if (newContent !== project.files?.[documentFile]?.content) {
            const updatedFiles = {
                ...(project.files || {}),
                [documentFile]: { content: newContent }
            };
...
export const DocumentWorkspace: React.FC<IdeWorkspaceProps> = (props) => {
    ...
    // On mobile, just show chat for simplicity
    if (isMobile) {
